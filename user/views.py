...
from innovation.models import News
...
@csrf_exempt
def user3(request):
    news_items = News.objects.all().order_by('-created_at')  # Eng yangi yangiliklar birinchi keladi
    paginator = Paginator(news_items, 4)  # Har bir sahifada 4 ta yangilik
    page_number = request.GET.get('page')  # URL'dan sahifa raqamini olish
    page_obj = paginator.get_page(page_number)  # Joriy sahifadagi elementlar

    return render(request, 'users/user3.html', {'news_items': page_obj})
...
def user1(request):
    ...
    return render(request, 'users/user1.html', {'news_items': page_obj})


@login_required
@csrf_exempt
def user2(request):  # "reqest" o'rniga "request" to'g'ri yozilishi kerak
    news_items = News.objects.all().order_by('-created_at')  # Eng yangi yangiliklar birinchi keladi
    paginator = Paginator(news_items, 4)  # Har bir sahifada 4 ta yangilik
    page_number = request.GET.get('page')  # URL'dan sahifa raqamini olish
    page_obj = paginator.get_page(page_number)  # Joriy sahifadagi elementlar

    return render(request, 'users/user2.html', {'news_items': page_obj})
